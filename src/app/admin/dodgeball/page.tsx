...
import type { RosterTeam, DodgeballMatch, SoloResult } from "@/lib/types";
...
export default function AdminDodgeballPage() {
  ...
  const groupStandings = useMemo(
    () =>
      dodge ? computeGroupStandings(dodge.groupMembers, dodge.matches, teams) : [],
    [dodge, teams]
  );

  const groupsLocked = dodge?.state?.groups_locked ?? false;
  const bracketSeeded = dodge?.state?.bracket_seeded ?? false;
  const groupsDone = dodge ? groupStageComplete(dodge.matches) : false;

  const qualifiers = useMemo(
    () => computeQualifiers(groupStandings, priorityTeamIds),
    [groupStandings, priorityTeamIds]
  );
  const wildcard = resolvedWildcard(qualifiers, dodge?.state ?? null);
  const four = resolvedQualifiers(qualifiers, dodge?.state ?? null);
  ...
  async function lockGroups() {
    ...
    if (
      !confirm(
        "Snapshot the current standings and generate dodgeball groups? Teams will be split by rank {1,6,7}/{2,5,8}/{3,4,9}."
      )
    )
      return;
    setBusy(true);
    ...
    const matchRows: Omit<DodgeballMatch, "id" | "created_at" | "updated_at">[] =
      [];
    ...
    await supabase.from("dodgeball_matches").insert(matchRows);

    await supabase
      .from("dodgeball_state")
      .upsert({ id: 1, groups_locked: true, updated_at: new Date().toISOString() });

    await logAudit(supabase, "create", "dodgeball_tournament", "1", {
      action: "lock_groups",
  ...
  async function resetTournament() {
    if (
      !confirm(
        "Reset the entire dodgeball tournament? All groups, matches and bracket results will be deleted."
      )
    )
      return;
    setBusy(true);
    const supabase = createClient();
    await supabase.from("dodgeball_matches").delete().gte("created_at", EPOCH);
    await supabase.from("dodgeball_group_members").delete().gte("created_at", EPOCH);
    await supabase.from("dodgeball_state").upsert({
      id: 1,
      groups_locked: false,
    ...
    await logAudit(supabase, "delete", "dodgeball_tournament", "1", {
      action: "reset",
    });
    await load();
    setBusy(false);
  ...
  async function saveMatch(match: DodgeballMatch, scoreA: number, scoreB: number) {
    if (scoreA === scoreB) {
      alert("A dodgeball match can't end in a tie — enter different round wins.");
      return;
    }
    setBusy(true);
    const supabase = createClient();
    ...
    await logAudit(supabase, "update", "dodgeball_match", match.id, {
      stage: match.stage,
  ...
  async function propagateBracket(supabase: ReturnType<typeof createClient>) {
    const { matches } = await fetchDodgeballData(supabase);
  ...
  async function chooseWildcard(teamId: string) {
    ...
    await logAudit(supabase, "update", "dodgeball_tournament", "1", {
      action: "set_wildcard",
  ...
  async function seedBracket() {
    ...
    const rows: Omit<DodgeballMatch, "id" | "created_at" | "updated_at">[] = [
      {
        stage: "semi",
        group_label: null,
        slot: 1,
    ...
    await supabase.from("dodgeball_matches").insert(rows);
    await supabase
      .from("dodgeball_state")
  ...
  const bracket = dodge ? bracketMatches(dodge.matches) : null;
      ...
      <div className="flex items-center gap-3 mb-8">
        ...
        <div>
          ...
          <p className="text-sm text-muted">
            Lock groups from the standings, record round wins, then seed the
            playoff bracket
          </p>
        </div>
        {groupsLocked && (
          ...
          <section>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
              {GROUP_LABELS.map((label) => {
                ...
                const groupMatches = (dodge?.matches ?? []).filter(
                  (m) =>
                    m.stage === "group" &&
                    !m.is_tiebreaker &&
                    m.group_label === label
...
function SectionTitle({
  ...
  return (
    <div className="flex items-center gap-3 mb-4">
      <span className="w-7 h-7 rounded-full bg-orange-500/10 text-orange-500 font-bold text-sm flex items-center justify-center">
        {step}
      </span>
      <div>
...
function LockGroupsSection({
  ...
  // Preview the snake seeding each team would land in.
  const preview = assignGroups(standings);
  return (
      ...
      <div className="px-5 py-4 border-b border-border flex items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-muted">
            Groups will be assigned by rank: {"{1,6,7}"} → A, {"{2,5,8}"} → B,{" "}
            {"{3,4,9}"} → C.
          </p>
        </div>
...
function MatchEditor({
  ...
  match: DodgeballMatch;
  teamById: Map<string, RosterTeam>;
  busy: boolean;
  onSave: (m: DodgeballMatch, a: number, b: number) => void;
}) {
