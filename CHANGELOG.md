...
- **The hero takes the room it's the centre of.** The screen's container has gone from 64rem to 72rem to 80rem and the marquee's cap grew with it each time, with the HUD bar widened to match so the cabinet's chrome still lines up. The attract line, the date board, PRESS START and the padding around all of them stepped up with it
...
### The cabinet is the whole screen, and it boots before it plays
- **The hero is the viewport**, less the header sitting above it. Arriving puts you in front of the cabinet rather than beside it, and the contents centre themselves in whatever is left of the screen after the HUD — so the marquee sits in the middle of a laptop window and a desktop one alike, rather than under a stack of padding picked for one of them. Measured in `svh`, so a phone's disappearing browser chrome doesn't leave the hero an address bar taller than the window
- The marquee grew with it: the cap is now **6rem**, which is twelve characters at 72rem inside an 80rem container, with the rest spare for the padding, the screen-printed shadow and the splash hanging off the corner
- **The first arrival in a tab gets a loading screen.** A cabinet doesn't come up in attract mode, it comes up loading: the wordmark, a twenty-cell bar filling left to right, a percentage counting up beside it, and the machine naming what it's doing — booting, loading sprites, warming phosphor, starting attract mode. It reaches READY, holds, and the power cuts
- **The power cut is a CRT losing its supply**, not a fade: the picture collapses into a bright line and the line goes out, in six frames of `steps()`. The page underneath was there the whole time
- **It plays once per visit**, keyed in `sessionStorage`. Coming back from the 2026 site, pressing Back, or looking at the page a second time goes straight to the marquee — a loader you sit through every time you press Back is a toll, not a flourish
- The screen is **server-rendered**, so it covers the page from the first frame rather than appearing a beat after React wakes up, by which point the visitor has already seen the hero and the loader is a lie. For anybody who has already seen it, a one-line inline script stamps the document as the parser goes past and CSS takes the screen out **before it is painted**, rather than mounting it and tearing it down. Nothing React owns is touched, so hydration never sees a node change under it
- **The bar is not measuring anything** and doesn't pretend to — the page is a static render and was ready before the bar started. The cadence is written out frame by frame, with the stalls a real loader has, and the honest thing to do with a loader that has nothing to wait for is keep it under two seconds
- The page **can't be scrolled behind it**, and under **reduced motion it never renders at all** — somebody asking for less motion is asking for less of exactly this. Screen readers get one sentence, once, instead of a counter shouting numbers over a page that has already loaded

### The archive steps back
- The stage select was competing with the marquee for the same attention, which is backwards on a page about the *next* event. It now sits in a **48rem column against the hero's 80rem**, a step down in every type size, in a hairline frame rather than an accent one. With the cabinet taking the whole viewport it is below the fold outright, which is the same decision made twice
